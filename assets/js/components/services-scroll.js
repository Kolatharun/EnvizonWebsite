...
const SERVICES = [
    {
        ...
        impact: 'Clients see an average 40% reduction in drop-off and 2.8× increase in task completion rates within 60 days of launch.',
        tags: ['User Research', 'Information Architecture', 'Interaction Design', 'Usability Testing']
    },
    {
        title: 'Web Design',
        description: 'A beautiful layout means nothing if it slows people down. We design pages that load fast, read clearly, and move visitors toward a decision on every scroll.',
        impact: 'Redesigned sites convert 35% more visitors on average, with bounce rate dropping within the first two weeks live.',
        tags: ['Visual Design', 'Responsive Layouts', 'Design Systems', 'Prototyping']
    },
    {
        title: 'Web Development',
        description: 'Design without solid engineering behind it breaks under real traffic. We build fast, resilient front ends that hold up as products and teams grow.',
        impact: 'Pages ship with sub-1.5s load times and 98+ Lighthouse scores, cutting infrastructure cost alongside bounce rate.',
        tags: ['Front-End Engineering', 'CMS Integration', 'Performance Tuning', 'API Integration']
    },
    {
        title: 'Branding & Identity',
        description: 'A logo is not a brand. We build identity systems that hold together across every touchpoint, so recognition compounds instead of resetting each time.',
        impact: 'Brands we relaunch report 3× higher recall in customer surveys within the first quarter.',
        tags: ['Logo Design', 'Brand Guidelines', 'Visual Identity', 'Art Direction']
    },
    {
        title: 'CRO Optimization',
        description: 'Traffic without conversion is just spend. We test what actually changes behavior, not what looks clever in a deck.',
        impact: 'Ongoing CRO programs typically lift conversion rate by 20-45% within two quarters of testing.',
        tags: ['A/B Testing', 'Funnel Analysis', 'Heatmap Tracking', 'Copy Optimization']
    },
    {
...
function renderPanels(track) {
    ...
    track.innerHTML = SERVICES.map((service, index) => `
...
export function initServicesScroll(sectionSelector = '#servicesScroll') {
    ...
    for (let i = 0; i < total - 1; i++) {
        ...
        if (i >= 1) {
            tl.to(panels[i - 1], { opacity: 0, duration: 0.5, ease: 'power1.out' }, i);
        }
